...
for i in range(0,10):
    #try:
    if isinstance(predseq['new_term_errors'],str)==0:
        ...
        #####TEMPORARY FIX FOR TERMINATOR STRUCTURES######
        if 1<0:
            print(len(whole_term))
            term_length = term_end-term_start
            new_term = predseq['new_term_structure'].iloc[i]
            whole_term = whole_term[0:term_start]+new_term[0:term_length]
            print(len(whole_term))
        ...
        output_file_term = 'tbox_images/termts/'+name+'_term.png '
        ...
        hr_te = hr_te+hr_s1+hr2#+hr_at+hr3+hr4
    ...
    #except:
        #print('Skipped: '+str(i))
         ...
         name=predseq['unique_name'].iloc[i]
         print('Skipped '+name+' due to error in structure')
